...
#	print(type(batch[1]))
	return out_batch
def eval_phones(test_path,rep_type, batch_size, num_epochs, inp_dim0, inp_dim1, model_path, 
		hidden_dim = 500, all_gpu=False):
	print('eval_phones MMM')
	'''
...
#	torch.set_num_threads(32)
	...
	# except:
	# 	# "fc1.weight", "fc1.bias", "fc.weight", "fc.bias". 
	...
	# 	# cuda = torch.cuda.is_available()
	# 	torch.cuda.set_device(0)  # set device to use
	...
	valid_phones = ['ao', 'ae', 'r', 'eh', 't', 'b', 'aa', 'f', 'k', 'ng', 's', 'g', 'ow', 'er'
		 , 'l', 'th', 'z', 'aw', 'd', 'dh', 'sh', 'hh', 'iy', 'ch', 'm', 'ey', 'v', 'y', 'zh', 
		 'jh', 'p', 'uw', 'ah', 'w', 'n', 'oy', 'ay', 'ih', 'uh']
...
#		print("----------- %s Pass seconds --------------" % (time.time() - start_time))
	...
	if not os.path.exists(main_path+'/PhoneProbes'):
		os.makedirs(main_path+'/PhoneProbes')
...
# parser.add_argument('--test_path',type=str,	help='path to validation tsv',
# 		     default=main_path+'/DeepSpeech/data/test_1750_small.csv')
...
# parser.add_argument('--model_path', type= str,  help='path to the Deepspeech Acoustic model',
# default=main_path+'/DeepSpeech/models/an4_pretrained_v3.ckpt')


# parser.add_argument('--rep_type', type=str, default=main_path+'/DeepSpeech/data/Contribution/MCV/conv/')


# MMM commented 
# parser.add_argument('--rep_type', type=str, default='rnn_0')
...
# parser.add_argument('--rep_type', type=str, default='', choices=rep_type)
...
args = parser.parse_args()
dim = {'spec':[161,1], 'conv':[1312,1], 'rnn_0': [1024,1], 'rnn_1': [1024,1],
        'rnn_2': [1024, 1], 'rnn_3': [1024, 1], 'rnn_4': [1024,1]}
